...
public class Boss : MonoBehaviour {
	...
	private float timeBetweenDropsStores;
	...
	void Start () {
		...
		timeBetweenDropsStores = timeBetweenDrops;
		bossRight = true;
	...
	void Update () {
		...
		if (theLevelManager.respwanCoActive) {
			bossActive = false;
			waitingForRespawn = true;
		}

		if (waitingForRespawn && !theLevelManager.respwanCoActive) {
			theBoss.SetActive (false);
			leftPlatforms.SetActive (false);
			rightPlatforms.SetActive (false);

			timeBetweenDrops = timeBetweenDropsStores;
